...
form.addEventListener('submit', (event) => {
    ...
    const Day = NUmber (document.getElementById('surname').value);
    const Month = Number (document.getElementById('surname').value);
    const Year = Number (document.getElementById('surname').value);

    const result = document.getElementById("result");


    const first_message = `Hello, ${Name} ${Surname}`;
    const second_message = `Your Zodsiac Sign is: ${zodiacSigns}`;
    const third_measage = compliments[Day-1];

    result.inner = Maths.floor(Math.random() * 20);
    const fourth_mesage = victimCardCompliments[index];
    

    index = (Name.length * Surname.length * Year)%30;
    const fifth_mesage = recommendations[index];

    index = (Day*Month*Year)%20;
    const sixth_mesage = predictions[index];

    result.innerHTML = `${first_message} <br> ${second_message} <br> ${third_measage} <br> ${fourth_mesage} <br> ${fifth_mesage} <br> ${sixth_mesage}`;
